...
const icons = {
  ...
  occupancy: <path d="M19 3H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm-2 10H7v-2h10v2zm0-4H7V7h10v2z"/>,
  ...
  const visibleNavGroups = useMemo(() => {
    return navGroups.map(grp => {
      const items = grp.items.filter(([k]) => {
        ...
        if (isStaff) return ['dashboard', 'sites', 'campaigns', 'campaign-details', 'occupancy', 'electricity', 'notifications', 'activity'].includes(k);
        if (isReadOnly) return ['dashboard', 'sites', 'campaigns', 'campaign-details', 'occupancy', 'storage', 'electricity', 'reports', 'notifications', 'activity'].includes(k);
        return true;
      });
      return { ...grp, items };
        ...
        <main className="scooh-main">
          <div className="scooh-view">
            <Routes>
              ...
              <Route path="/occupancy" element={<Navigate to="/campaign-details" replace />} />
...
/* ─────────────────────────────────────────────────────────────────────────
   Campaign Tracker / Occupancy -> Redirected to Campaign Details
   ───────────────────────────────────────────────────────────────────────── */
function OccupancyView() {
  return <Navigate to="/campaign-details" replace />;
}
        ...
        <div className="scooh-sectionbar" style={{ padding: '14px 20px' }}>
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
            <button
              ...
              title="Filter and view all bookings by client, display and dates in Campaign Details"
            >
